...
namespace GatheringTimer.Data.ThirdParty.CafeMaker
{
    public class CafeMakerUpdater : BaseUpdater
    {
        ...
        public override async Task Sync(CancellationToken cancellationToken)
        {
            ...
            {
                foreach (Model.Entity.Item item in Container.Container.GetContainer().Resolve<IGatheringTimerResource>().ItemCache)
                {
            ...
            cacheTasks.Enqueue(Task.Run(() =>
            {
                foreach (Model.Entity.SpearfishingItem spearfishingItem in Container.Container.GetContainer().Resolve<IGatheringTimerResource>().SpearfishingItemCache)
                {
            ...
            cacheTasks.Enqueue(Task.Run(() =>
            {
                foreach (Model.Entity.PlaceName placeName in Container.Container.GetContainer().Resolve<IGatheringTimerResource>().PlaceNameCache)
                {
            ...
            updateTasks.Enqueue(Container.Container.GetContainer().Resolve<IGatheringTimerResource>().GetSQLiteDatabase().UpdateRowByRow<Model.Entity.Item>(Container.Container.GetContainer().Resolve<IGatheringTimerResource>().ItemCache, new List<string> { "Name_chs" }, new List<string> { "ID" }, cancellationToken));
            updateTasks.Enqueue(Container.Container.GetContainer().Resolve<IGatheringTimerResource>().GetSQLiteDatabase().UpdateRowByRow<Model.Entity.SpearfishingItem>(Container.Container.GetContainer().Resolve<IGatheringTimerResource>().SpearfishingItemCache, new List<string> { "Description_chs" }, new List<string> { "ID" }, cancellationToken));
            updateTasks.Enqueue(Container.Container.GetContainer().Resolve<IGatheringTimerResource>().GetSQLiteDatabase().UpdateRowByRow<Model.Entity.PlaceName>(Container.Container.GetContainer().Resolve<IGatheringTimerResource>().PlaceNameCache, new List<string> { "Name_chs" }, new List<string> { "ID" }, cancellationToken));
            await Task.WhenAll(cacheTasks);
            ClearCache();
